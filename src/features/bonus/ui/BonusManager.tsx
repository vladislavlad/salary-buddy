...
import type { BonusType, LocalDate, BonusUpdateRequest } from "@/shared/types";
  ...
  const handleSave = async (data: {
    ...
    const result = data.id
      ? await updateBonus({
          ...
          amount: data.amount,
          type: data.type,
        } satisfies BonusUpdateRequest)
      : await addBonus({ date: data.date, amount: data.amount, type: data.type });
    ...
    <div className="space-y-4">
      ...
      {!addingNew ? (
        ...
        <div className="rounded-lg border bg-card-secondary p-3">
          <BonusForm onSave={handleSave} onCancel={() => setAddingNew(false)} />
        </div>
      )}
    </div>
  );
