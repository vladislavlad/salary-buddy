...
import type { SalarySettings, PaymentDistribution, SalaryChange } from '@/types';
...
export function SalaryManager({ settings, onChange }: SalaryManagerProps) {
  const [editingId, setEditingId] = useState<string | null>(null);
  const [addingNew, setAddingNew] = useState(false);

  // Сортируем оклады по дате
  const sortedChanges = [...settings.salaryChanges].sort(
    (a, b) => a.effectiveDate.getTime() - b.effectiveDate.getTime()
  );

  const handleAddChange = (data: Omit<SalaryChange, 'id'> & { id?: string }) => {
    ...
    const updated = data.id
      ? settings.salaryChanges.map((c) => (c.id === data.id ? newChange : c))
      : [...settings.salaryChanges, newChange];
    onChange({ salaryChanges: updated });
    setEditingId(null);
    setAddingNew(false);
  };
  ...
  const handleDeleteChange = (id: string) => {
    onChange({
      salaryChanges: settings.salaryChanges.filter((c) => c.id !== id),
    });
  };
      ...
      <div className="space-y-3">
        ...
        {!addingNew ? (
          ...
          <div className="rounded-lg border bg-card-secondary p-3">
            <SalaryChangeForm onSave={handleAddChange} onCancel={() => setAddingNew(false)} />
          </div>
        )}
      </div>
    </div>
